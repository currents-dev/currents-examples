...
There’s a third project (`e2e-03`) that uses a different Nx target (`or8n`) that runs `npx pwc-p run` (Currents orchestration command) via `nx:run-commands`. :contentReference[oaicite:7]{index=7}
...
- **Multiple Nx Playwright projects**
  ...
  - `apps/e2e-03` with `or8n` target running `npx pwc-p run`. :contentReference[oaicite:13]{index=13}

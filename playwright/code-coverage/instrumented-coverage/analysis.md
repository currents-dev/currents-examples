...
- Run one of the provided scripts:
  ...
  - `npm run test:e2e:pwc-p` → starts Next dev server + runs `pwc-p run` (orchestrated). :contentReference[oaicite:8]{index=8}
- Coverage plumbing:
...
- **Runnable scripts** showing 3 execution modes:
  - `playwright`, `pwc`, and `pwc-p run` (all via `start-server-and-test` against `http://localhost:3000`). :contentReference[oaicite:12]{index=12}
